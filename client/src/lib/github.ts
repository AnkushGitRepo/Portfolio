...
export async function fetchGitHubRepos(
  ...
  try {
    ...
    const excludedRepos = [
      'MERN-Sandbox', 'MERN Sandbox',
      'DSA',
      'AnkushGitRepo',
      'Pharmacy-Management-System',
      'Currency_Converter_Using_Core_Java', 'Currency Converter'
    ];
    repos = repos.filter(repo => !excludedRepos.some(excluded =>
      repo.name.toLowerCase() === excluded.toLowerCase() ||
      repo.name.toLowerCase().replace(/-/g, ' ') === excluded.toLowerCase() ||
      repo.name.toLowerCase().replace(/_/g, ' ') === excluded.toLowerCase()
    ));
  ...
  return repos.map(repo => {
    ...
    if (repo.name === 'Portfolio') {
      return {
        ...
        technologies: ['Next.js', 'React', 'TypeScript', 'Express', 'MongoDB', 'Tailwind CSS'],
    ...
    if (repo.name === 'DataHarbor') {
      return {
        _id: repo.id.toString(),
        title: 'DataHarbor',
        description: repo.description || 'A comprehensive data management and analytics platform. DataHarbor provides tools for data collection, processing, and visualization.\n\nBuilt with modern technologies, this project demonstrates advanced data handling capabilities and user-friendly interfaces.',
        image: `${getAssetPath('/images/projects/Dataharbor_1.png')}, ${getAssetPath('/images/projects/Dataharbor_2.png')}, ${getAssetPath('/images/projects/Dataharbor_3.png')}`,
        technologies: repo.topics?.length > 0 ? repo.topics.map(topic => topic.charAt(0).toUpperCase() + topic.slice(1)) : (repo.language ? [repo.language] : ['Python', 'Data Science', 'Analytics']),
        githubUrl: repo.html_url,
    ...
    const technologies = repo.topics?.length > 0
    ...
    return {
      ...
      description: repo.description || `A ${repo.language || 'code'} repository.`,
      image: dummyImage,
      technologies,
      githubUrl: repo.html_url,
      liveUrl: repo.homepage || '',
